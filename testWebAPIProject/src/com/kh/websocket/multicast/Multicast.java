...
// configurator : 서버 설정에 관한 부분으로 서버 개통 시에 기본적으로 해야할 설정 정보들을 담는 객체
...
public class Multicast {
   ...
   public static Set<Session> getUserList(String room){
       ...
       // 만약에 사용자 정보가 없다면 새로 생성하여 사용자를 받을수 있도록 객체를 생성해준다
       if(userList == null) {
   ...
   // 연결하려는 사용자의 정보와 접속한 방의 이름을 받아서 해당 방의 사용자 리스트에 갱신을 해주어야 한다
    ...
    public void onMessage(String message, Session session) throws IOException {
    	// 현재 사용자가 보낸 정보를 같은 방의 다른 상ㅇ자들에게 전달하는 메소드
      ...
      // 같은 방 안늬 사용자들에게 데이터를 뿌려주는 forEach메소드 작성
      userList.stream().forEach( x -> {
    	  // 람다식 사용부 :
    	  // 기존 정차지향 언어가 가지는 특성인 익명 함수 구현 부분을 객체 지향언어에서도 흉내내기 위한 소스코드 작성 기술
    	  // 쉽게 이해하기 위해 JavaScript의 function(X){ ... } :: --> (X) -> { ... } 를 생각해보자!
    	  // X : Session
         try {
        	 // 실제 데이터를 전달하는 로직 부분
            if(!x.equals(session))
